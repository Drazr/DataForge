...
|---|---|
| 0:00–0:35 | Introduce the need: receiving and confirming appointment details by voice, with the ability to interrupt and ask again. Identify all appointments as synthetic. |
...
| 1:35–2:35 | Start a new session. Interrupt during a detail and say "repeat the time." Show that unfinished information is preserved and that interruption alone does not confirm. |
...
| 3:20–4:10 | Export evidence. Show actual measured interruption timing and the separate cached/uncached observations from the successful acceptance run. If no measured run exists, this segment is not ready. |
