...
| 16 | A long run can lose a Colab session. | Save one JSON row per clip, reuse verified rows on rerun, and estimate runtime from a 12-score preflight. | Implemented in Cells 8–9. |
...
| 19 | GPU quota may be unavailable for the 294-score core. | Run the pinned Faster-Whisper model with CPU INT8 and rescore every clean/noisy v2 row on the same backend. This removes GPU dependence while preserving within-run comparisons; Cell 8 measures the slower runtime before the full run. | Implemented; both notebooks now use CPU runtimes. |
| 20 | Cell 8 clipped on the second preflight synthesis because −26 dBFS plus a high-crest-factor noise window exceeded the float-mixture peak limit at −5 dB SNR. | Before creating the run, evaluate the exact 14 cached phone waveforms against every frozen noise/SNR combination and choose one global 0.5 dB-stepped level below a 0.90 peak ceiling. This preserves SNR and avoids per-clip limiting or silent normalization. | Implemented in Cell 7; the failed partial run remains separate under its old fingerprint. |
