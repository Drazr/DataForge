...
| 13 | The first CPU preflight clipped at −5 dB because the selected noise window's crest factor made the fixed −26 dBFS mixture exceed the peak limit. | Audit every exact planned mixture before scoring and choose one global 0.5 dB-stepped speech level below a 0.90 peak ceiling. This preserves SNR and avoids per-clip limiting. | Fixed at `0474bb3`; rerun producer Cells 7–8 to create the new run. |
