...
## Current implementation (supersedes the initial plan below)

User-reported or explicitly test-injected risk now requires exact code/time
read-back before confirmation. One retry is allowed; two wrong replies end
unconfirmed. Missing automatic detection is labeled not assessed and does not
trigger this flow by itself. Automatic competing-speech detection is not installed.

Catalog methods in use: LiveKit turn-taking/transport, phrase/audio caching, and
disclosed fallback. Risk-aware confirmation and critical-fact read-back are
test-motivated product policies, not items named in the catalog. Controller,
runtime, API, UI and live-test fixtures implement the flow. 81 software checks,
TypeScript, and production build passed; credentialed live results are pending.

This supporting plan is temporarily archived on main; build from final-product.
